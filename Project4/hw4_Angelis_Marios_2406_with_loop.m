...
for d=1:1:length(M_vector)
  ...
  for k=1:1:length(SNR_db)
    ...
    %OFDM encoder
    index=1;
    for r=1:ofdm_subcarriers:length(total_tx_out)
      
      
      %x=fftshift(total_tx_out(r:r+ofdm_subcarriers-1),64);
      ofdm_idft_out=ifft(total_tx_out(r:r+ofdm_subcarriers-1),ofdm_subcarriers);
      
      ofdm_with_cyclic_prefix=[ofdm_idft_out ofdm_idft_out(1:cyclic_prefix)];
      ...
      if strcmp(channel_type,"AWGN")==1
        ...
        channel_out=conv(ofdm_with_cyclic_prefix,h);
      endif
      ...
      awgn_out_without_cyclic_prefix=awgn_out(1:ofdm_subcarriers);
      ofdm_dft_out=fft(awgn_out_without_cyclic_prefix,ofdm_subcarriers);
      
      if strcmp(channel_type,"AWGN")==1
        equalizer_out(index:index+ofdm_subcarriers-1)=ofdm_dft_out;
      else
        equalizer_out(index:index+ofdm_subcarriers-1)=ofdm_dft_out./H;
      endif
    ...
    decoded_symbol_vector=real_equalizer_out + j*imag_equalizer_out;
